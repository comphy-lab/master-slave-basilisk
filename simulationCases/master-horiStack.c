...
u.n[left]  = dirichlet(slave_interpolate ("u.x", 0.0, y, 0, true));
u.t[left]  = dirichlet(slave_interpolate ("u.y", 0.0, y, 0, true));
p[left]    = neumann(0.);
pf[left]   = neumann(0.);
...
event movies (t += 0.05; t <= 30.)
{
  ...
  output_ppm (vel, file = "vel-master-horiStack.mp4",
